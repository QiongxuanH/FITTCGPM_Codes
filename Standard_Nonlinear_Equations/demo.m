...
% set parameters for inertial derivative-free projection methods (FITTCGPMs)
...
para2.tau = 0.7;         % the compression ratio
% para1.alpha = 0.1;       % the coefficient of inertial step
...
for index=1:np
    ...
    for repeats = 1:5
        if (Num<=27 && Num>=23)
            ...
            x0 = 2*rand(n,1);    % x0\in (0,10). x0 = 4*rand(n,1)时间占优，但有一个问题不可解
        end
        [T1,NFF1,NI1,G1] = IDFPM(Num,'MITTCGP',1,4,x0,para3); % acceleration
        [T2,NFF2,NI2,G2] = IDFPM(Num,'FITTCGPM-PRP',2,1,x0,para3);
        [T3,NFF3,NI3,G3] = IDFPM(Num,'FITTCGPM-DY',2,1,x0,para3);
...
legend('MITTCGP','FITTCGPM-PRP','FITTCGPM-DY','ISTCP','Location','southeast');
% %subplot(2,2,2);
figure(2);
perf(F,'logplot');
%title('目标函数计算性能');
