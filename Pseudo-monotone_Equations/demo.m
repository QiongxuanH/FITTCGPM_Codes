...
%     fprintf(fid_tex,'%s & %d/%d/%.3f/%.2e & %d/%d/%.3f/%.2e\n& %d/%d/%.3f/%.2e & %d/%d/%.3f/%.2e\\\\ \r\n', ... 
    ...
    [T3,NFF3,NI3,G3] = IDFPM(Num,'FITTCGPM-DY',2,1,x0,para1);
    [T4,NFF4,NI4,G4] = ISTCP(Num,'ISTCP',2,2,x0,para2);
   fprintf(fid_tex,'%s & %d/%d/%.3f/%.2e & %d/%d/%.3f/%.2e & %d/%d/%.3f/%.2e & %d/%d/%.3f/%.2e\\\\ \n', ...               
                name,NI1,NFF1,T1,G1,NI2,NFF2,T2,G2,NI3,NFF3,T3,G3,NI4,NFF4,T4,G4);
    T(index,:) = [T1,T2,T3,T4];
    F(index,:) = [NFF1,NFF2,NFF3,NFF4];
    N(index,:) = [NI1,NI2,NI3,NI4];
%     G(index,:) = [G1,G2,G3,G4];
end
%% 关闭文件
fclose(fid_tex);
